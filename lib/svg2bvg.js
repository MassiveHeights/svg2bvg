...
const tagMap = {
  'rect'    : {convert: shapeCmds.RECT, converter: RectConverter, data: ['x', 'y', 'width', 'height', 'rx', 'ry']},
  ...
  linearGradient: {
    convert  : shapeCmds.LINEAR_GRADIENT,
    converter: LinearGradientConverter,
    data     : ['x1', 'y1', 'x2', 'y2'],
  },
...
const tagSupport = {
  ...
  polygon       : {},
  linearGradient: {},
  stop          : {data: ['offset', 'stop-color']},
  pattern       : {data: ['x', 'y', 'width', 'height']},
};
...
module.exports.SVG2BVG = class SVG2BVG {
  static convert(svgText) {
    ...
    Object.keys(converter.validate($node)).forEach(err => console.error(err));
  ...
  validate($node, errors = {}) {
    ...
    if (s) {
      ...
      Object.keys($node).forEach(attr => {
        if (attr.indexOf(':') !== -1 || attrSupport[attr] || styleMap[attr] !== undefined) return;

        const data = (tagMap[$node.$type] && tagMap[$node.$type].data) || tagSupport[$node.$type].data || [];

        if (data !== 'all' && data.indexOf(attr) === -1) {
          errors[`${attr} attribute for ${$node.$type} tag is not supported`] = 1;
        }
      });

      $node.$nodes && $node.$nodes.forEach($node => this.validate($node, errors));
    } else {
      errors[`${$node.$type} tag name is not supported`] = 1;
    }
